...
MainWindow::MainWindow(QWidget* parent) :
  ...
  updateGUI("/sys/module/mpcubic/parameters/alpha");
  updateGUI("/sys/module/mpcubic/parameters/beta");
  updateGUI("/sys/module/mpcubic/parameters/fast_convergence");
  updateGUI("/sys/module/mpcubic/parameters/tcp_friendliness");

  // Location of the TCP Evil Parameters
  watcher.addPath("/sys/module/mpcubic/parameters/alpha");
  watcher.addPath("/sys/module/mpcubic/parameters/beta");
  watcher.addPath("/sys/module/mpcubic/parameters/fast_convergence");
  watcher.addPath("/sys/module/mpcubic/parameters/tcp_friendliness");
...
void MainWindow::updateGUI(const QString& str) {
  string value;
  int i_value;
  if (str.toStdString() == "/sys/module/mpcubic/parameters/alpha") {
    value = exec("cat /sys/module/mpcubic/parameters/alpha");
    value.pop_back(); // remove the newline
    i_value = stoi(value);
    ui->alpha_value->setValue(i_value);
  }
  else if (str.toStdString() == "/sys/module/mpcubic/parameters/beta") {
    value = exec("cat /sys/module/mpcubic/parameters/beta");
    value.pop_back(); // remove the newline
    i_value = stoi(value);
    ui->beta_value->setValue(i_value);
  }
  else if (str.toStdString() ==
           "/sys/module/mpcubic/parameters/fast_convergence") {
    value = exec("cat /sys/module/mpcubic/parameters/fast_convergence");
    value.pop_back(); // remove the newline
    i_value = stoi(value);
    if (i_value == 1) {
      ...
      ui->chk_fast_convergence->setChecked(false);
           ...
           "/sys/module/mpcubic/parameters/tcp_friendliness") {
    value = exec("cat /sys/module/mpcubic/parameters/tcp_friendliness");
    value.pop_back(); // remove the newline
    i_value = stoi(value);
    if (i_value == 1) {
...
void MainWindow::on_chk_fast_convergence_toggled(bool checked) {
  ...
  if (checked) {
    ...
    e = system("echo -n 0 > /sys/module/mpcubic/parameters/fast_convergence");
  }
  if (DEBUG && e != 0) {
...
void MainWindow::on_chk_tcp_friendliness_toggled(bool checked) {
  int e;
  if (checked) {
    e = system("echo -n 1 > /sys/module/mpcubic/parameters/tcp_friendliness");
  }
  else {
    e = system("echo -n 0 > /sys/module/mpcubic/parameters/tcp_friendliness");
  }
  if (DEBUG && e != 0) {
...
void MainWindow::on_slider_alpha_valueChanged(int value) {
  ...
  stringstream ss;
  ss << "echo -n " << value <<
    " > /sys/module/mpcubic/parameters/alpha";
...
void MainWindow::on_slider_beta_valueChanged(int value) {
  int status;
  stringstream ss;
  ss << "echo -n " << value << " > /sys/module/mpcubic/parameters/beta";
  status = system(ss.str().c_str());
  if (DEBUG && status != 0) {
...
void MainWindow::on_actionAbout_TCPvil_triggered() {
  QMessageBox msgBox;
  msgBox.setText("mpCUBIC: An improved version of CUBIC");
